...
async def _(event):
    ...
    packname = f"piu sticker Pack"
    packshortname = f"piu sp"  # format: Uni_Borg_userid
    ...
    if is_a_s:
        ...
        packname = f"piu animated sp"
        if userid == 719877937:
            packshortname = "SnapDragon_Animated"
        else:
            packshortname = f"piu asp" # format: Uni_Borg_userid
    ...
    await event.edit("liking this sticker👍 Please Wait!")
    ...
    await event.edit(f"sticker liked! check all other liked stickers [here](t.me/addstickers/{packshortname})")
